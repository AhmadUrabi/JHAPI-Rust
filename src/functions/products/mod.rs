...
pub async fn get_product_by_supplier(supplier_id: i32, pool: &Pool, sql_manager: &SQLManager, key: &ApiKey<'_>) -> Result<Vec<Product>, APIErrors> {
    let username = decode_token_data(key.0).unwrap().USER_ID.unwrap();
    
    let mut store_ids: HashSet<String> = HashSet::new();
    let sql = "SELECT * FROM ODBC_JHC.JHC_INVDATA WHERE SUPPLIER_ID = :id";
    match get_stores(&pool, &sql_manager, username).await {
        Ok(store_list) => {
            for i in store_list {
                store_ids.insert(i.STORE_ID.unwrap());
    ...
    let mut rows = stmt.query(&[&supplier_id]).map_err(|e| {
        error!("Error executing query: {:?}", e);
        APIErrors::DBError
    })?;
